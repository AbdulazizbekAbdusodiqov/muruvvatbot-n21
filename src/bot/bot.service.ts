...
export class BotService {
      ...
      if (!user) {
        ...
        if (user.last_state === 'location') {
          if ('location' in ctx.message!) {
            ...
            const oldmsg = await ctx.reply(
              "...",
              Markup.removeKeyboard(),
            );

            await ctx.deleteMessage(oldmsg.message_id)

            ctx.reply("Bo'limlardan birini tanlang: ", {
              reply_markup: {
                inline_keyboard: [
                  [{ text: "Murojat yo'llash", callback_data: `murojat_yollash__${user_id}` }]
                ]
              }
            })
  ...
  async onPhoto(ctx: Context) {
    try {
      ...
      if (!user) {
        ...
        if (user.last_state == "photo") {
          ...
          if (ctx.message && 'photo' in ctx.message) {
            ...
            const uniqueFileName = Date.now() + fileId + '.jpg';
            ...
            console.log("sho'tta qotdi");
            const newImage = await UserImg.create({ user_id: user_id!, img_url: uniqueFileName })
            if (newImage) {
              await ctx.reply(`Rasm saqlandi. Fayl nomi: ${newImage.img_url}`);
      ...
      console.log("onPhoto: ", error);
    }
  }

  async onClickMurojatYollash(ctx: Context) {
    try {
      const callbackData = 'data' in ctx.callbackQuery! ? ctx.callbackQuery.data : '';
      const [_, user_id] = callbackData.split('__');
      ...
      if (!user) {
        ...
        user.last_state = "photo"
        await user.save()

        ctx.reply("Iltimos Rasmingizni yuboring: ",
          Markup.removeKeyboard()
        )

      }
    } catch (error) {
      console.log("onClickMurojatYollash: ", error);
